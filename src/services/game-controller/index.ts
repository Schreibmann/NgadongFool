...
import { CardDeck36 } from "@shared-constants";
...
export class GameController {
  ...
  sort(cards: Card[]) {
    cards
      ...
      .sort((a, b) =>
        a.suit !== this.trump && b.suit !== this.trump ? a.rank - b.rank : 1,
      )
      .sort((a, b) =>
        a.suit === this.trump && b.suit === this.trump ? a.rank - b.rank : 0,
      );
  }
  ...
  async passCard({
    ...
    fromPlayer.cards = fromPlayer.cards.filter(
    ...
    if (this.current.length === this.players.length) {
      ...
      if (fromPlayer.cards.length === 0) {
        if (fromPlayer.stumps.length > 0) {
          ...
          console.log(`${fromPlayer.name} quit game`);
  ...
  async mainGameCpuPassBadCard(player: Player) {
    ...
    const spades = player.cards.filter(
      (card) => card.suit === CardSuits.Spades,
    );
  ...
  async prepareTurnPlayer({ target, card }: { target: Player; card: Card }) {
    ...
    if (!this.activePlayer.canContinueTurn && this.deck.length === 1) {
      ...
      this.trump =
        this.deck[0].suit === CardSuits.Spades ? null : this.deck[0].suit;
    ...
    let canPassToSomeOne =
      this.checkIfCanPassCard(opponents[0], card) ||
      this.checkIfCanPassCard(opponents[1], card);
  ...
  async prepareTurnCPU(playerId: number, playerCard?: Card) {
    ...
    if (canPass) {
      if (playerCard) {
        ...
        if (skip) {
          console.log(
            `${this.activePlayer.name} skip pass card to ${receiverId}`,
          );
          self.strikes++;
        } else {
  ...
  checkIfCanPassCardMain(targetCard: Card, card: Card) {
    ...
    const canPass =
      (sameSuit && card.rank > targetCard.rank) ||
      (!targetIsSpade && !targetIsTrump && isTrump);
    return canPass;
  }
  ...
  findCardToPass(player: Player) {
    ...
    const validCards = player.cards.filter((card) =>
      this.checkIfCanPassCardMain(targetCard, card),
    );
  ...
  async setNextPlayer() {
    ...
    const nextPlayer = {
      ...(this.players[nextPlayerIndex] || this.players[0]),
    };
